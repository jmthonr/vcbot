...
async def tg_dl(event):
    jmthon = await edit_or_reply(event, "**- جار التحميل**")
    name = NAME
    path = None
    ...
    if reply:
        ...
        if (
            ...
            await reply.download_media(
                ...
                progress_callback=lambda d, t: asyncio.get_event_loop().create_task(
                    progress(d, t, jmthon, c_time, "trying to download")
                ),
            )
        ...
        await jmthon.edit(
            f"**- الوقت المستغرق للتحميل: {ms} م الثواني.**\n**تم التحميل الى :- **  `{os.path.relpath(file_name,os.getcwd())}`\n"
        )
        return os.path.relpath(file_name, os.getcwd())
    else:
        await jmthon.edit("**- يجب عليك الرد على الميديا لتحميلها وتشغيلها**")
        return False
